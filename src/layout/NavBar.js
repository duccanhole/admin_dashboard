    ...
    return (
        <div className='d-flex flex-row shadow background'>
            ...
            <Offcanvas show={show} onHide={handleClose} className='sidebar'>
                ...
                <Offcanvas.Body>
                    <Nav className="flex-column">
                        <Link className='text-light text-decoration-none nav-item text-center nav-item mt-3 hvr-underline-from-center' to='/'>Home</Link>
                        ...
                        {route.map((item, index) => {
                                if(role.includes(item.roleId)) return (
                                    <Link className='text-light text-decoration-none nav-item text-center nav-item mt-3 hvr-underline-from-center' to={item.route} key={index}>{item.category_name}</Link>
                                )
                                return null;
                            })}
                    </Nav>
            ...
            <Offcanvas show={true}
                ...
                name='Enable body scrolling' scroll={true} backdrop={false}>
                <Offcanvas.Body>
                    <div className='d-flex flex-column flex-shrink-0'>
                        <Nav className="flex-column">
                            <OverlayTrigger placement='right'
                                overlay={<Tooltip id="button-tooltip">Home</Tooltip>}>
                                <Link className='text-light nav-item text-center text-center fs-3 btn hvr-underline-from-center' to='/'><BsHouseDoorFill /></Link>
                            </OverlayTrigger>
                            {/*<OverlayTrigger placement='right'
