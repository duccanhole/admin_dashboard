...
import { setTotal as setTotalDataP, setLoading as setLoadingDataP }
    from '../app/productReducer';
import { setTotal as setTotalDataO, setLoading as setLoadingDataO }
    from '../app/orderReducer';
import { setTotal as setTotalDataA, setLoading as setLoadingDataA }
    from '../app/accountReducer';
import axios from 'axios';
import { toast } from 'react-toastify';
//api
import { productURL, authURL, orderURL } from '../api/config';
...
export default function GeneralInfor() {
    ...
    const [totalProducts, setTotalProducts] = useState(useSelector(state => state.products.total));
    const [totalAccounts, setTotalAccounts] = useState(useSelector(state => state.accounts.total));
    const [totalOrders, setTotalOrders] = useState(useSelector(state => state.orders.total));
    ...
    return (
        <div className='d-flex flex-wrap'>
            <div className='mx-2 text-center p-3 rounded inforBox bg-box mt-1 flex-fill'>
                <h4 className='text-light'><BsFillArchiveFill /> Total products</h4>
                <hr className='text-light'></hr>
                <h3 className='text-info'>
                    {loading?<Spinner animation="grow" variant="info"/>:totalProducts}
                </h3>
            </div>
            <div className='mx-2 text-center p-3 rounded inforBox bg-box mt-1 flex-fill'>
                <h4 className='text-light'><BsFillBagFill /> Total orders</h4>
                <hr className='text-light'></hr>
                <h3 className='text-info'>
            ...
            <div className='mx-2 text-center p-3 rounded inforBox bg-box mt-1 flex-fill'>
                <h4 className='text-light'><BsPeopleFill /> Total accounts</h4>
                <hr className='text-light'></hr>
                <h3 className='text-info'>
                    {loading?<Spinner animation="grow" variant="info"/>:totalAccounts}
                </h3>
            </div>
        </div>
    );
