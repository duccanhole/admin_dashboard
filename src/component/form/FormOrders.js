...
import { orderURL } from "../../api/config.js";
...
export default function FormOrders(props) {
    ...
    const handleSubmit = async(e)=>{
        ...
        if(!validatePhoneNumber(phone)){
            toast.warning('Phone number is invalid (only contain number and start with 0)',{
                position: 'top-center'
            });
            setLoading(false);
            return;
        ...
        <Form className="text-start" onSubmit={handleSubmit}>
            <Form.Group>
                ...
                <Form.Control type="text"
                    ...
                    placeholder="Enter your phone number..." minLength={10} required />
                ...
                <Form.Control type="number"
                    ...
                    placeholder="Enter value..." required min={0} />
                <div className='mt-2 d-flex justify-content-center'>
                    <Button variant="primary" type="submit" className='w-75 mx-auto' disabled={loading}>
                        {!loading ? <>Update</> : <Spinner as="span" animation="border" role="status" aria-hidden="true" />}
                    </Button>
                </div>
                <div className='mt-2 d-flex justify-content-center'>
                    <Button variant="secondary" type="button" className='w-75 mx-auto' onClick={() => props.cancel()}>
                        Cancel
                    </Button>
                </div>
            </Form.Group>
