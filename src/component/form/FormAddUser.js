...
import { authURL } from '../../api/config.js';
...
export default function FormAddUser(props) {
    ...
    const handleSubmit = async(e)=>{
        e.preventDefault();
        setLoading(true);
        const message = props.type==='update'?validateAccount(phone, null, null):validateAccount(phone, confirmPassword, password);
        if(message !== '') {
            ...
            <Form.Group>
                ...
                <div className='mt-2 d-flex justify-content-center'>
                    <Button variant="primary" type="submit" className='w-75 mx-auto' disabled={loading}>
                        {!loading ? <>Add/Update</> : <Spinner as="span" animation="border" role="status" aria-hidden="true" />}
                    </Button>
                </div>
                <div className='mt-2 d-flex justify-content-center'>
